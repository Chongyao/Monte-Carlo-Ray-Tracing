...
vec radiance(const Ray &r, int depth, unsigned short *Xi, const Scene &scene, const vector<unique_ptr<KD_tree_tris>>& KD_forest) {
  int mtl_id;   // id of material of intersected triangle
  Ray next;     // intersection point and normal
  //TODO: replace with my intersect
  if (!r.intersect_forest(KD_forest, mtl_id, next)) return {}; // return black
...
int main(int argc, char *argv[]) {
  ...
  for(size_t m_id = 0; m_id < num_model; ++m_id){
    ...
    for(size_t f_id = 0; f_id < num_tris; ++f_id){
      childrens[f_id] = std::move(make_shared<tri_aabb>(scene.models[m_id]->tris_[f_id].mtl_id_, scene.models[m_id]->tris_[f_id].p_));
    }
      ...
      for (int sy = 0, i = (st.h - y - 1) * st.w + x; sy < 2; sy++) { // 2x2 subpixel
        for (int sx = 0; sx < 2; sx++, r = vec()) {
          for (int s = 1; s <= samps; s++) {
            ...
            // r = r + radiance(Ray(st.cam.o, d.norm()), 0, Xi, scene) * (1. / samps);
          }
